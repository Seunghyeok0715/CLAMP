...
class CLAMP(nn.Module):
    ...
    def sample(self, model, x_start, operator, measurement, evaluator=None, record=False, verbose=False, **kwargs):
        ...
        try:
            for step in pbar:
                ...
                Delta, g, x0hat, eps = self._penalized_gn_step(
                    x_ref=xt,
                    y=measurement,
    ...
    def _penalized_gn_step(self, *, x_ref, y, sigma_prior, x_eval_fn=None):
        ...
        return self._gmres_solve(matvec, (-c).detach()), g.detach(), x0_hat.detach(), eps_t.detach()

    def _gmres_solve(self, matvec, b: torch.Tensor) -> torch.Tensor:
        device = b.device
        dtype = b.dtype
        shape = b.shape
        ...
        v = [r0 / beta]
        h = torch.zeros((self.cg_iter + 1, self.cg_iter), device=device, dtype=dtype)
